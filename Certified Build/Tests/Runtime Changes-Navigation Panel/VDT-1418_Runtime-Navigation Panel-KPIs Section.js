...
test
    ...
    .click(Selector('#sandbox-host .icon.icon--ChevronRight.navPanelLabelPaddingRight').nth(4))
    ...
    .click(Selector('#sandbox-host span').withText('N9: Total Cost'))
    await doVisualRegression(t.testRun.test.testFile.currentFixture.name, t.testRun.test.name, 'KPISelection')
    await t
    ...
    .click(Selector('#N9').find('g.d2VDTNodeToggle').find('path.d2VDTToggle'))
    .click(Selector('div.d2VDTSection.periods').find('div.navToggle'))
    await doVisualRegression(t.testRun.test.testFile.currentFixture.name, t.testRun.test.name, 'SelectedKPI')
