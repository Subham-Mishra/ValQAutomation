...
test
    ...
    ('Key Inputs-Check Simulation', async t => {
    await t
    .maximizeWindow()
    ...
    await simulate()
    await doVisualRegression(t.testRun.test.testFile.currentFixture.name, t.testRun.test.name, 'Nav_Panel')
    await model()
    await t
    ...
    await doVisualRegression(t.testRun.test.testFile.currentFixture.name, t.testRun.test.name, 'Key_Inputs')
    await t
    ...
    .click(Selector('div.d2VDTNavCtr').find('div.d2VDTSection.assump').find('div.bf-ui-flex.sectionActions').find('div.d2VDTIconButton.editInputs'))
    .click(Selector('#sandbox-host .cell-view').nth(5))
    .click(Selector('#sandbox-host .cell-view').nth(5),{
    caretPos: 0
    })
    .typeText(Selector('div.grid-data-container').find('div.grid-cell').nth(5).find('input').withAttribute('type','text'), '25')
    .click(Selector('.grid-fixed-virtualized'))
    ...
    await model()
    await t
    .doubleClick(Selector('#sandbox-host div').withText('N1: Revenue').nth(22))
    .click(Selector('#modal-root span').withText('Simulation'))
    .click(Selector('#modal-root div').withText('Growth Percentage').nth(13))
    ...
    await t.click(Selector('div.d2VDTSection.assump').find('div.navToggle'))
    await doVisualRegression(t.testRun.test.testFile.currentFixture.name, t.testRun.test.name, 'Toggle_Clicked')
